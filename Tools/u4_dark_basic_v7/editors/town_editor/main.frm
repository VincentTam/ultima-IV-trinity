...
Begin VB.Form main 
   ...
   Begin VB.Menu filex 
      Caption         =   "File"
      Begin VB.Menu open_load8bit 
         Caption         =   "&Import 8bit (*.ULT)"
      End
      Begin VB.Menu open_load 
         Caption         =   "&Import 16bit (*.ULT)"
      End
      Begin VB.Menu savex 
         Caption         =   "&Update 16bit (*.ULT)"
      End
      Begin VB.Menu saveas 
         Caption         =   "&Save As 16bit (*.ULT)"
      End
      Begin VB.Menu endx 
         Caption         =   "Quit"
      End
...
blah = BitBlt(main.dest.hDC, (icon_x * 24 - 24), (icon_y * 24 - 24), 24, 24, main.icon_slctd.hDC, 0, 0, &HCC0020)
main.dest.Refresh
End If
...
icon_selected = 0
X1 = Int(icon_selected / 25) + 1
Y1 = Int((icon_selected / 15) * 15) - ((X1 - 1) * 15) + 1
blah = BitBlt(main.icon_slctd.hDC, (1 * 24 - 24), (1 * 24 - 24), 24, 24, icons.src.hDC, X1 * 24 - 24, Y1 * 24 - 24, &HCC0020)
...
Get #1, b, aword: town(Y, X) = aword:: b = b + 2
Next X: Next Y: b = b - 1
...
For Y = 1 To 32
Get #1, b + Y, character
town_npc(Y, 1) = character.id: n = b + Y
Next Y: b = n

'import 32 bytes of start_X
For Y = 1 To 32
Get #1, b + Y, character
town_npc(Y, 2) = character.id: n = b + Y
Next Y: b = n

'import 32 bytes of start_Y
For Y = 1 To 32
Get #1, b + Y, character
town_npc(Y, 3) = character.id: n = b + Y
Next Y: b = n
...
For Y = 1 To 32
Get #1, b + Y, character
town_npc(Y, 4) = character.id: n = b + Y
Next Y: b = n

'64bytes of blank bytes
For Y = 1 To 64: Get #1, b + Y, character: n = b + Y: Next Y: b = n

'import 32 bytes of AGGRO motion status
For Y = 1 To 32
Get #1, b + Y, character
town_npc(Y, 5) = character.id: n = b + Y
'Debug.Print character.id
Next Y: b = n
'Stop
...
For Y = 1 To 32
Get #1, b + Y, character
town_npc(Y, 6) = character.id: n = b + Y
Next Y: b = n
Close
...
blah = BitBlt(main.dest.hDC, (X * 24 - 24), (Y * 24 - 24), 24, 24, main.icon_slctd.hDC, 0, 0, &HCC0020)
Next X: Next Y
main.dest.Refresh
End Sub
...
'SAVE town structure/tiles as WORD (2 bytes)
b = 1: For Y = 1 To 32: For X = 1 To 32
aword = town(Y, X)
'byte_lo = Int(aword / 255)
'byte_hi = ((aword / 255) - byte_lo) * 255
...
'SAVE 32 bytes of AGGRO motion status
For Y = 1 To 32
...
Private Sub open_load8bit_Click()
Call clear_array
